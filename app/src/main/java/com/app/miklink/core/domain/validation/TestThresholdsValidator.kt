...
object TestThresholdsValidator {
    ...
    fun validate(thresholds: TestThresholds) {
        ...
        require(isValidNonNegative(thresholds.speed.minDownloadMbps)) { "Invalid minimum download threshold" }
        require(isValidNonNegative(thresholds.speed.minUploadMbps)) { "Invalid minimum upload threshold" }
    }
}
